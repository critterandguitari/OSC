...
{
  return (int)(UART2_BUFFER_SIZE + uart2_recv_buf_head - uart2_recv_buf_tail) % UART2_BUFFER_SIZE;
}
...
int uart2_read(void)
{
  ...
  if (uart2_recv_buf_head == uart2_recv_buf_tail) {
    ...
    uart2_recv_buf_tail = (unsigned int)(uart2_recv_buf_tail + 1) % UART2_BUFFER_SIZE;
    return c;
  }
}
